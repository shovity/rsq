...
class Queue {
  ...
  newTopic(name, options) {
    const topic = new Topic( name, { redisClient: this.redisClient, queueName: this.name, ...options || {} })
  ...
  push(message) {
    ...
    if (topic) {
      ...
      console.log(`"${message.topic}" not match any topic, message lost`)
    }
  }
  ...
  /**
   * Auto clean prefix queue_
   */
  clean(callback) {
    this.redisClient.keys('queue_*',  (err, reply) => {
