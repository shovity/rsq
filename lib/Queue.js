...
class Queue extends IEx{
  /**
   * @param {Object} config
   *  if redisClient is undefined, create redisClient with redisConfig
   */
  constructor(name, config = {}) {
    ...
    // pass event emiter to down
    this.config.on = this.on
    this.config.emit = this.emit
  ...
  push(message, callback) {
    ...
    if (topic) {
      topic.push(message, callback)
    } else {
      done(new Error(`"${message.topic}" not match any topic, message was lost`))
    }
  }
